...
We implemented our mixed-dimensional modeling strategy in open-source parallel software known as Amanzi-ATS~\cite{ref}. Amanzi-ATS is the result of extending the flow and reactive transport  simulator Amanzi~\cite{moulton2012high} through the addition of an advanced multiphysics management system~\cite{ecoon2016managing} to create a runtime-configurable system, where multiple instances of process kernels representing individual ecohydrological processes can be hierarchically coupled to create models with complex spatial structures using standard or user-defined coupling schemes. The initialism ATS may refer to either the Advanced Terrestrial Simulator, which is the general capability, or the Arctic Terrestrial Simulator, which is one particular configuration~\cite{spainter2106integrated}, depending on context. The Arctic Terrestrial Simulator configuration solves strongly coupled surface energy balance, and surface and subsurface thermal hydrology with freeze/thaw dynamics. This work extends the ATS to work with a multicolumn spatial structure. 
